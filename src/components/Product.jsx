...
const Product = ({item}) => {
  ...
  return (
    ...
    hover:scale-110 transition duration-300 ease-in gap-3 p-4 mt-10 ml-5 rounded-xl  shadow-[rgba(0,_0,_0,_0.4)_0px_30px_90px]">
      <div className="text-gray-700 font-semibold text-lg text-left truncate w-40 mt-1">
        <p>{item.title.substr()}</p>
      </div>
      <div className="w-40 text-gray-400 font-normal text-[10px] text-left">
        <p>{item.description.split(" ").slice(0,10).join(" ") + "..."}
        </p>
      </div>
      <div className="h-[180px]">
        <img src={item.image} className="h-full w-full"/>
      </div>
      <div className="flex justiify-between gap-12 items-center w-full mt-5">
        <div>
